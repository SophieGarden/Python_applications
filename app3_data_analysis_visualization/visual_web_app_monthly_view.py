...
df_month_average = df.groupby('month').mean()
...
chart_def = """
{
    ...
    xAxis: {
        ...
        accessibility: {
            rangeDescription: 'Month of year'
        },
        maxPadding: 0.05,
        showLastLabel: true
    },
    ...
    tooltip: {
        ...
        pointFormat: '{point.x} Month: {point.y} rating'
    },
    plotOptions: {
        spline: {
            marker: {
...
def app():
    ...
    hc.options.xAxis.categories = list(df_month_average.index)
    hc.options.series[0].data = list(df_month_average.Rating)
